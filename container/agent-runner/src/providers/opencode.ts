...
function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  const providerOptions: Record<string, unknown> =
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            // Force the OpenAI-*compatible* SDK (Chat Completions API). The
            // catalog "openai" provider uses @ai-sdk/openai, which defaults to
            // the Responses API (POST /responses) — Vertex's OpenAI-compat
            // endpoint (OpenMaaS) rejects that for first-party Gemini with
            // "model not supported". @ai-sdk/openai-compatible only ever calls
            // POST /chat/completions, which Vertex accepts.
